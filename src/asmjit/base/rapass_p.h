...
struct RARegStats {
  ...
  ASMJIT_INLINE bool makeClobbered(uint32_t kind) noexcept { _packed |= Utils::mask(kIndexClobbered + kind); }
  ...
  ASMJIT_INLINE bool makePrecolored(uint32_t kind) noexcept { _packed |= Utils::mask(kIndexPrecolored + kind); }
  ...
  ASMJIT_INLINE bool makeUsed(uint32_t kind) noexcept { _packed |= Utils::mask(kIndexUsed + kind); }
